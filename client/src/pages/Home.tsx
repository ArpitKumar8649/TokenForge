  ...
  return (
    <div className="aurora-home">
      ...
      <main>
        ...
        <section className="aurora-model-band"><div className="aurora-model-band__inner"><div><div className="aurora-kicker"><span /> Curated by verification</div><h2>{TOKENFORGE_MODELS.length} sharp<br /><em>ways to think.</em></h2><p className="aurora-model-intro">Every published counter represents successful, provider-reported tokens for that exact text-chat route.</p></div><div className="aurora-model-list">{TOKENFORGE_MODELS.map((model, index) => <article key={model.id}><span>{String(index + 1).padStart(2, "0")} / {model.provider}</span><h3>{model.name}</h3><p>{model.eyebrow}</p><strong><AnimatedTokenCount value={metrics.data?.byModel[model.id as keyof typeof metrics.data.byModel] ?? 0} /> <small>tokens processed</small></strong></article>)}</div><div className="aurora-model-metric" aria-live="polite"><i /><span>Network total</span><b><AnimatedTokenCount value={metrics.data?.totalTokens ?? 0} /> tokens</b><small>Successful provider-reported tokens · refreshes every 20 seconds</small></div></div></section>
