...
const CLUSTER_MESSAGES_MODEL_IDS = new Set([
  ...
  "kimi-k3", "gemini-2.5-flash", "gemini-2.5-flash-lite", "mistral-large-3", "mistral-small-4", "minimax-m2", "minimax-m2-7", "qwen3.7-max", "claude-opus-5", "qwen3.8-27b", "qwen3.8-max", "claude-fable-5",
]);
    ...
    <div className="public-page tf-public-surface docs-page">
      ...
      <main className="docs-shell">
        <aside className="docs-sidebar">
          <p>Get started</p>
          <a href="#overview">Overview</a><a href="#authentication">Authentication</a><a href="#completions">Chat completions</a><a href="#claude-code">Claude Code</a><a href="#claude-code-models">Messages models</a><a href="#models">Models</a><a href="#limits">Request admission</a><a href="#errors">Errors</a>
        ...
        <article className="docs-content">
          ...
          <p className="docs-lead">TokenForge uses familiar OpenAI-compatible request shapes, precise error responses, and transparent credit settlement so you spend less time adapting infrastructure.</p>
          ...
          <section id="claude-code" className="docs-section"><h2><Terminal size={19} /> Claude Code compatibility</h2><p>TokenForge also supports Anthropic-style <code>POST /v1/messages</code> requests for text and tool workflows from Claude Code. This endpoint accepts your TokenForge key through <code>x-api-key</code> or Bearer authentication and applies the same credit, model-control, and usage-log protections as the OpenAI-compatible gateway.</p><p>Claude Opus 5, Claude Fable 5, Qwen3.8 27B, and Qwen 3.8 Max use TokenForge’s server-side Messages translation route, preserving compatible text and tool workflows while retaining protected gateway controls. The bridge accepts Claude Code thinking-history blocks without forwarding private reasoning as ordinary prompt text. Image and document blocks are not supported.</p><div className="code-panel"><div className="code-panel__bar"><span>Claude Code shell configuration</span></div><pre><code>{claudeCodeExample}</code></pre></div></section>
          ...
          <section id="models" className="docs-section"><h2>Models & credit rates</h2><p>Use <code>GET /v1/models</code> to discover models currently enabled for the gateway. TokenForge currently exposes {TOKENFORGE_MODELS.length} verified text-chat routes. Model pages show the final TokenForge credit rate, including the <strong>3.5</strong> TokenForge platform charge. Image, audio, embedding, transcription, and modality-specific routes remain excluded.</p><div className="docs-model-badges">{TOKENFORGE_MODELS.map(model => <Badge key={model.id} variant="secondary">{model.id}</Badge>)}</div><p>Credits are reserved from the maximum requested output allowance before execution, then settled from reported input and output tokens only after a successful request. Failed or cancelled requests release their reservation and receive no usage debit. See the <a href="/models">model catalogue</a> for final TokenForge credit rates and model details.</p></section>
          <section id="limits" className="docs-section"><h2>Request admission</h2><p>TokenForge does not impose daily request, daily token, per-minute, or concurrent-request admission limits. You may make simultaneous API and Playground requests while your account is active. Model availability, platform maintenance, upstream-provider behavior, and your promotional-credit balance can still affect whether a request is accepted.</p></section>
          <section id="errors" className="docs-section"><h2>Errors</h2><p>Responses use stable JSON error types and include an <code>x-request-id</code> for tracing. TokenForge does not emit platform rate-limit headers or reject calls because of platform request-rate, daily quota, or concurrency caps.</p><div className="error-table"><div className="error-table__head"><span>Code</span><span>HTTP</span><span>Meaning</span></div>{errorRows.map(row => <div className="error-table__row" key={row[0]}><code>{row[0]}</code><span>{row[1]}</span><p>{row[2]}</p></div>)}</div></section>
        </article>
      </main>
      <PublicFooter />
    </div>
