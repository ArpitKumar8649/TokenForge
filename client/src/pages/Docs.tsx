    ...
    <div className="public-page tf-public-surface docs-page">
      ...
      <main className="docs-shell">
        ...
        <article className="docs-content">
          ...
          <section id="models" className="docs-section"><h2>Models</h2><p>Use <code>GET /v1/models</code> to discover models currently enabled for the gateway. TokenForge currently exposes {TOKENFORGE_MODELS.length} verified text-chat routes, each with an exact first-party input and output rate. Image, audio, embedding, transcription, and modality-specific routes remain excluded.</p><div className="docs-model-badges">{TOKENFORGE_MODELS.map(model => <Badge key={model.id} variant="secondary">{model.id}</Badge>)}</div><p>See the <a href="/models">model catalogue</a> for source-linked rates and provider details.</p></section>
