...
const CLUSTER_MESSAGES_MODEL_IDS = new Set([
  ...
  "kimi-k3", "gemini-2.5-flash", "gemini-2.5-flash-lite", "mistral-large-3", "mistral-small-4", "minimax-m2", "minimax-m2-7", "qwen3.7-max",
]);
    ...
    <div className="public-page tf-public-surface docs-page">
      ...
      <main className="docs-shell">
        ...
        <article className="docs-content">
          ...
          <section id="claude-code" className="docs-section"><h2><Terminal size={19} /> Claude Code compatibility</h2><p>TokenForge also supports Anthropic-style <code>POST /v1/messages</code> requests for text and tool workflows from Claude Code. This bridge accepts your TokenForge key through <code>x-api-key</code> or Bearer authentication, applies the same credit, quota, rate-limit, model-control, usage-log, and server-only Cluster Protocol credential controls as the OpenAI-compatible gateway.</p><p>Use an enabled <strong>Cluster Protocol</strong> model identifier such as <code>kimi-k3</code>. The Messages endpoint rejects FXQidian and TokenHarbor models before any provider request. Image and document blocks are not supported by this bridge.</p><div className="code-panel"><div className="code-panel__bar"><span>Claude Code shell configuration</span></div><pre><code>{claudeCodeExample}</code></pre></div></section>
          <section id="claude-code-models" className="docs-section"><h2>Messages-supported models</h2><p><code>POST /v1/messages</code> supports the {CLAUDE_CODE_MESSAGES_MODELS.length} Cluster Protocol routes listed below. Each must also be enabled in the live TokenForge catalogue; paused models return a clear availability error rather than silently falling back. Other TokenForge providers are intentionally not accepted on this endpoint.</p><div className="docs-model-badges">{CLAUDE_CODE_MESSAGES_MODELS.map(model => <Badge key={model.id} variant="secondary" title={model.name}>{model.id}</Badge>)}</div></section>
