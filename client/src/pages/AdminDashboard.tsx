...
function OrcaRouterCredentialPanel() {
  ...
  const normalizedValues = values.map(value => value.trim()) as [string, string, string];
...
export default function AdminDashboard() {
  ...
  const [section, setSection] = useState<"overview" | "accounts" | "controls" | "orcarouter" | "announcement" | "activity">("overview");
  ...
  const body = loading ? <div className="grid h-80 place-items-center"><Loader2 className="animate-spin text-[#c9ff73]" /></div> : overview.error ? <div className="dashboard-card grid min-h-64 place-items-center p-6 text-center"><div><AlertTriangle className="mx-auto text-[#f0c180]" size={22} /><p className="mt-3 text-sm font-semibold text-white">Live operations data could not load</p><p className="mt-2 max-w-sm text-xs leading-5 text-[#9091a3]">Your administrator session is still active. Refresh the control-plane data to try again.</p><Button size="sm" variant="outline" className="mt-4 border-white/12 text-[#e2e1ea] hover:bg-white/10" onClick={() => overview.refetch()}>Retry live data</Button></div></div> : <div className="space-y-4">{metrics}<div className="dashboard-card flex flex-wrap gap-2 p-2">{(["overview", "accounts", "controls", "orcarouter", "announcement", "activity"] as const).map(item => <Button key={item} size="sm" variant={section === item ? "default" : "ghost"} onClick={() => setSection(item)} className={section === item ? "bg-[#c9ff73] text-[#17210d] hover:bg-[#d8ff91]" : "text-[#b7b8c7] hover:bg-white/8 hover:text-white"}>{item === "overview" ? "Overview" : item === "accounts" ? "Accounts" : item === "controls" ? "Model controls" : item === "orcarouter" ? "OrcaRouter keys" : item === "announcement" ? "Announcement" : "Activity"}</Button>)}</div>{section === "overview" && <div className="grid gap-4 xl:grid-cols-2">{accountSection}<section className="dashboard-card"><SectionHeader icon={<ChartNoAxesCombined size={17} />} title="Usage activity" detail="Recent request activity across the TokenForge gateway." /><AdminUsageChart usage={overview.data?.usage ?? []} /></section><section className="dashboard-card"><SectionHeader icon={<Mail size={17} />} title="Email provider distribution" detail="Live account counts by mailbox domain. Individual email addresses are never displayed." /><div className="mt-5"><EmailProviderChart providers={emailProviders} loading={overview.isLoading} /></div></section><div className="xl:col-span-2">{capacitySection}</div></div>}{section === "accounts" && accountSection}{section === "controls" && controlSection}{section === "orcarouter" && <OrcaRouterCredentialPanel />}{section === "announcement" && announcementSection}{section === "activity" && activitySection}</div>;
