...
export default function AdminDashboard() {
  ...
  const [fableApiKeys, setFableApiKeys] = useState(["", "", "", "", ""]);
  const fableSettings = trpc.admin.claudeFable5ProviderSettings.useQuery(undefined, { enabled: isAdminSession });
  const [opusBaseUrl, setOpusBaseUrl] = useState("");
  const [opusModel, setOpusModel] = useState("");
  const [opusApiKeys, setOpusApiKeys] = useState(["", "", "", "", "", "", ""]);
  ...
  useEffect(() => { if (fableSettings.data) { setFableBaseUrl(fableSettings.data.baseUrl); setFableModel(fableSettings.data.model); } }, [fableSettings.data]);
  useEffect(() => { if (opusSettings.data) { setOpusBaseUrl(opusSettings.data.baseUrl); setOpusModel(opusSettings.data.model); } }, [opusSettings.data]);
  const saveFableSettings = trpc.admin.updateClaudeFable5ProviderSettings.useMutation({
    onSuccess: async () => { setFableApiKeys(["", "", "", "", ""]); await Promise.all([utils.admin.claudeFable5ProviderSettings.invalidate(), utils.admin.activity.invalidate()]); toast.success("Claude Fable 5 provider settings updated"); },
    onError: error => toast.error(error.message),
  });
  const saveOpusSettings = trpc.admin.updateClaudeOpus5ProviderSettings.useMutation({
    onSuccess: async () => { setOpusApiKeys(["", "", "", "", "", "", ""]); await Promise.all([utils.admin.claudeOpus5ProviderSettings.invalidate(), utils.admin.activity.invalidate()]); toast.success("Claude Opus 5 provider settings updated"); },
    onError: error => toast.error(error.message),
  });
  const opusProviderSettingsSection = <section className="dashboard-card"><SectionHeader icon={<KeyRound size={17} />} title="Claude Opus 5 runtime settings" detail="The isolated seven-slot route uses encrypted server configuration. TokenForge identity protection is enforced separately on every API and Playground request." /><div className="mt-5 grid gap-5 xl:grid-cols-[minmax(0,1fr)_minmax(18rem,.8fr)]"><div className="space-y-4"><div><label className="text-xs font-semibold text-[#dedfe7]" htmlFor="opus-base-url">Base URL</label><Input id="opus-base-url" value={opusBaseUrl} onChange={event => setOpusBaseUrl(event.target.value)} className="mt-2 border-white/10 bg-black/15 font-mono text-xs" disabled={opusSettings.isLoading || saveOpusSettings.isPending} /></div><div><label className="text-xs font-semibold text-[#dedfe7]" htmlFor="opus-model">Upstream model ID</label><Input id="opus-model" value={opusModel} onChange={event => setOpusModel(event.target.value)} className="mt-2 border-white/10 bg-black/15 font-mono text-xs" disabled={opusSettings.isLoading || saveOpusSettings.isPending} /></div><div><p className="text-xs font-semibold text-[#dedfe7]">API keys</p><p className="mt-1 text-[10px] text-[#858697]">Leave any field blank to retain the existing server-only credential. Plaintext values are never returned to this page.</p><div className="mt-3 grid gap-2">{opusApiKeys.map((value, index) => <div key={index} className="grid gap-2 sm:grid-cols-[5rem_minmax(0,1fr)] sm:items-center"><span className="text-[10px] font-bold uppercase tracking-[.12em] text-[#858697]">Key {index + 1}</span><Input type="password" value={value} onChange={event => setOpusApiKeys(keys => keys.map((key, slot) => slot === index ? event.target.value : key))} placeholder={opusSettings.data?.apiKeyMasks[index]?.value ?? "Not configured"} className="border-white/10 bg-black/15 font-mono text-xs placeholder:text-[#9b9dac]" autoComplete="new-password" disabled={saveOpusSettings.isPending} /></div>)}</div></div><Button onClick={() => saveOpusSettings.mutate({ baseUrl: opusBaseUrl, model: opusModel, apiKeys: opusApiKeys })} disabled={saveOpusSettings.isPending || !opusBaseUrl.trim() || !opusModel.trim()} className="bg-[#c9ff73] text-[#17210d] hover:bg-[#d8ff91]">{saveOpusSettings.isPending ? <><Loader2 className="animate-spin" size={14} />Saving…</> : "Save provider settings"}</Button></div><div className="rounded-xl border border-white/8 bg-black/15 p-4"><p className="text-[10px] font-bold uppercase tracking-[.13em] text-[#858697]">Current backend configuration</p><Badge className="mt-3 border-0 bg-white/6 text-[10px] text-[#d5d6df]">{opusSettings.data?.source === "database" ? "Admin-managed override" : "Environment default"}</Badge><dl className="mt-5 space-y-4 text-xs"><div><dt className="text-[#858697]">Base URL</dt><dd className="mt-1 break-all font-mono text-[#e6e6ee]">{opusSettings.data?.baseUrl ?? "Loading…"}</dd></div><div><dt className="text-[#858697]">Model ID</dt><dd className="mt-1 break-all font-mono text-[#e6e6ee]">{opusSettings.data?.model ?? "Loading…"}</dd></div><div><dt className="text-[#858697]">Credential slots</dt><dd className="mt-2 space-y-1">{opusSettings.data?.apiKeyMasks.map(item => <p key={item.slot} className="font-mono text-[#c9ff73]">Key {item.slot}: {item.value}</p>) ?? <p className="text-[#858697]">Loading…</p>}</dd></div></dl><p className="mt-5 border-t border-white/8 pt-4 text-[10px] leading-5 text-[#858697]">Partial saves preserve untouched settings. The public Claude Opus 5 identity policy remains server-enforced for Playground, OpenAI-compatible, and Anthropic-compatible calls; provider details are never returned to users.</p></div></div></section>;
  ...
  const providerSettingsSection = <section className="dashboard-card"><SectionHeader icon={<KeyRound size={17} />} title="Provider runtime settings" detail="Claude Fable 5 is routed through NVIDIA NIM. API keys are encrypted at rest and are displayed only as masked references." /><div className="mt-5 grid gap-5 xl:grid-cols-[minmax(0,1fr)_minmax(18rem,.8fr)]"><div className="space-y-4"><div><label className="text-xs font-semibold text-[#dedfe7]" htmlFor="fable-base-url">Base URL</label><Input id="fable-base-url" value={fableBaseUrl} onChange={event => setFableBaseUrl(event.target.value)} className="mt-2 border-white/10 bg-black/15 font-mono text-xs" disabled={fableSettings.isLoading || saveFableSettings.isPending} /></div><div><label className="text-xs font-semibold text-[#dedfe7]" htmlFor="fable-model">Upstream model ID</label><Input id="fable-model" value={fableModel} onChange={event => setFableModel(event.target.value)} className="mt-2 border-white/10 bg-black/15 font-mono text-xs" disabled={fableSettings.isLoading || saveFableSettings.isPending} /></div><div><p className="text-xs font-semibold text-[#dedfe7]">API keys</p><p className="mt-1 text-[10px] text-[#858697]">Leave a field blank to retain its current server-only credential. Existing values are never returned to the browser.</p><div className="mt-3 grid gap-2">{fableApiKeys.map((value, index) => <div key={index} className="grid gap-2 sm:grid-cols-[5rem_minmax(0,1fr)] sm:items-center"><span className="text-[10px] font-bold uppercase tracking-[.12em] text-[#858697]">Key {index + 1}</span><Input type="password" value={value} onChange={event => setFableApiKeys(keys => keys.map((key, slot) => slot === index ? event.target.value : key))} placeholder={fableSettings.data?.apiKeyMasks[index]?.value ?? "Not configured"} className="border-white/10 bg-black/15 font-mono text-xs placeholder:text-[#9b9dac]" autoComplete="new-password" disabled={saveFableSettings.isPending} /></div>)}</div></div><Button onClick={() => saveFableSettings.mutate({ baseUrl: fableBaseUrl, model: fableModel, apiKeys: fableApiKeys })} disabled={saveFableSettings.isPending || !fableBaseUrl.trim() || !fableModel.trim()} className="bg-[#c9ff73] text-[#17210d] hover:bg-[#d8ff91]">{saveFableSettings.isPending ? <><Loader2 className="animate-spin" size={14} />Saving…</> : "Save provider settings"}</Button></div><div className="rounded-xl border border-white/8 bg-black/15 p-4"><p className="text-[10px] font-bold uppercase tracking-[.13em] text-[#858697]">Current backend configuration</p><Badge className="mt-3 border-0 bg-white/6 text-[10px] text-[#d5d6df]">{fableSettings.data?.source === "database" ? "Admin-managed override" : "Environment default"}</Badge><dl className="mt-5 space-y-4 text-xs"><div><dt className="text-[#858697]">Base URL</dt><dd className="mt-1 break-all font-mono text-[#e6e6ee]">{fableSettings.data?.baseUrl ?? "Loading…"}</dd></div><div><dt className="text-[#858697]">Model ID</dt><dd className="mt-1 break-all font-mono text-[#e6e6ee]">{fableSettings.data?.model ?? "Loading…"}</dd></div><div><dt className="text-[#858697]">Credential slots</dt><dd className="mt-2 space-y-1">{fableSettings.data?.apiKeyMasks.map(item => <p key={item.slot} className="font-mono text-[#c9ff73]">Key {item.slot}: {item.value}</p>) ?? <p className="text-[#858697]">Loading…</p>}</dd></div></dl><p className="mt-5 border-t border-white/8 pt-4 text-[10px] leading-5 text-[#858697]">Partial saves preserve every untouched setting. Changes take effect on the next Claude Fable 5 request and are written to the administrator audit trail without key material.</p></div></div></section>;
