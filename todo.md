...
- [x] Locate Diego OmniRoute, create a separate private GitHub repository, and initialize an isolated workspace without changing TokenForge.
...
- [x] Retrieve and classify the seven reported recent Bailu provider failures using only credential-redacted administrative diagnostics; all seven were retryable SOCKS5 HostUnreachable transport rejections with no provider HTTP response.
- [x] Reproduce and fix the live Bailu SOCKS5 HostUnreachable defect by comparing real TokenForge request construction and stream handling against the successful isolated probe, without cross-provider fallback; switch Bailu Direct routing to proxy-side DNS resolution and verify JSON and SSE requests through all three currently enabled slots returned HTTP 200.
...
- [x] Add optional priority-routing mode across all managed multi-provider models, with validated provider priority numbers, lowest-number-first routing and compatible failover, an equal-share fallback toggle, protected administrator controls, regression coverage, validation, and publication.
- [x] Repair provider priority controls across all managed multi-provider models; add accessible drag-and-drop reordering, a working one-click restore-equal-share action, complete Claude Opus Qwen-pool participation, interaction regressions, validation, and publication.
- [x] Disable provider-priority inputs while equal-share mode is active, prevent refreshes from overwriting unsaved priority changes, provide explicit save guidance, add regressions, validate, and publish.
