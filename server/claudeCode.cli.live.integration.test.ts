...
describe("Claude Code CLI TokenForge gateway compatibility", () => {
  ...
  glmToolProbeIt("runs a real Claude Code tool continuation through the local GLM 5.3 Messages bridge", async () => {
    ...
    const { stdout, stderr } = await execFileAsync(
      "claude",
      ["-p", "Use your Bash tool to run exactly: printf TOKENFORGE_GLM53_TOOL_OK. After the tool succeeds, reply with exactly: TOKENFORGE_GLM53_TOOL_OK", "--output-format", "json", "--dangerously-skip-permissions"],
      {
        cwd: "/tmp",
        env: {
          ...process.env,
