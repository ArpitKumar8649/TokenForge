...
describe("NVIDIA Claude Fable 5 credential pool", () => {
  it("authorizes a minimal OpenAI-compatible completion with every configured Fable credential", async () => {
    ...
    for (const [slot, secret] of NVIDIA_CLAUDE_FABLE5_API_KEYS.entries()) {
      ...
      console.info("[NVIDIA Claude Fable 5 lightweight completion]", { slot: slot + 1, status: response.status, accepted: response.ok });
      expect(response.ok, `NVIDIA Claude Fable 5 credential slot ${slot + 1} was rejected with HTTP ${response.status}.`).toBe(true);
    }
  }, 155_000);
});
